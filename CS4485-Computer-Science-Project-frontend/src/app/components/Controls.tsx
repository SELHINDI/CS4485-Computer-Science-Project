        ...
        <div className="flex items-center gap-2">
          ...
          {[2, 3, 4, 8, 12].map(h => (
            <button
              ...
              className={`px-3 py-1 rounded-md border focus:outline-none focus:ring-2 focus:ring-primary ${horizons.includes(h)
                ? 'bg-secondary text-neutral-900 border-secondary'
                : 'bg-white dark:bg-black border-neutral-400 dark:border-neutral-400 text-neutral-900 dark:text-white hover:bg-neutral-100 dark:hover:bg-neutral-900'
                }`}
            >{h}</button>
          ))}
        </div>
